...
def inspect(args, world):
    ...
    print('-' * 40)
    print('server: % 22s\n' % address)
    print('map:        % 18s' % world.get_map().name)
    print('weather:    % 18s\n' % weather)
    print('time:       % 18s\n' % elapsed_time)
    ...
    if s.fixed_delta_seconds is None:
        ...
        fr = ('%.2f ms' % (1000.0 * s.fixed_delta_seconds),
            ' (%d FPS)' % (1.0 / s.fixed_delta_seconds))
    print('frame rate: % 18s%s' % fr)
    print('rendering:  % 18s' % ('disabled' if s.no_rendering_mode else 'enabled'))
    print('sync mode:  % 18s\n' % ('disabled' if not s.synchronous_mode else 'enabled'))
    print('actors:     % 18d' % len(actors))
    print('  * spectator:% 16d' % len(actors.filter('spectator')))
    print('  * static:   % 16d' % len(actors.filter('static.*')))
    print('  * traffic:  % 16d' % len(actors.filter('traffic.*')))
    print('  * vehicles: % 16d' % len(actors.filter('vehicle.*')))
    print('  * walkers:  % 16d' % len(actors.filter('walker.*')))
    print('-' * 40)
